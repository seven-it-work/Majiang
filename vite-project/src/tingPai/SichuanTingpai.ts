import {allDistintPai, Tingpai} from "./Tingpai.ts";
import {TingpaiTool} from "../util/TingpaiTool.ts";
import {TingDetail} from "./entities/TingDetail.ts";
import {RemoveTripleResult} from "./entities/RemoveTripleResult.ts";
 ...
 * 四川麻将听牌类
 */
export class SichuanTingpai implements Tingpai {
    public isTingCurrentPai(shoupai: number[], currentPai: number): TingDetail[] {
        const distinctShoupai = Array.from(new Set(shoupai));
        const tingDetails: TingDetail[] = [];
    ...
    private hupaiJudge(currentPai: number, toRemoveCopule: number, removeTripleResult: RemoveTripleResult): TingDetail | null {
        ...
        if (isAllTripleContinues.alltripleContinues) {
            ...
            const tingDetail = new TingDetail(
                isAllTripleContinues.triples,
                new DoublePia([toRemoveCopule, toRemoveCopule]),
                currentPai,
                true
            );
            return tingDetail;
    ...
    private hupaiJudgePre(currentPai: number, toRemoveCopule: number, removedCoupleShoupai: number[]): TingDetail | null {
        ...
        if (removeTripleContinuesResult.alltripleContinues) {
            const tingDetail = new TingDetail(
                removeTripleContinuesResult.triples,
                new DoublePia([toRemoveCopule, toRemoveCopule]),
                currentPai,
                true
            );
    ...
    public tingPais(shoupai: number[]): AllTingpai {
        ...
        const tingpais: number[] = [];
        const tingDetailsAll: TingDetail[] = [];
