...
export class TingpaiTool {
     ...
     * @returns 是否连续
     */
    static isThreeIntContinus(threeInt) {
     ...
     * @returns 是否相同
     */
    static isThreeIntTheSame(threeInt) {
        if (threeInt.length !== 3) {
            throw new Error('参数必须为三个');
        }
     ...
     * @returns 已经移除对子的手牌
     */
    static findshoupaiWithouCouple(shoupai, removeInt) {
        let result = [...shoupai];
        let count = 0;
     ...
     * @returns 已经移除三个的手牌
     */
    static findShoupaiWithoutTriple(shoupai) {
     ...
     * @returns 返回剩下的单个
     */
    static removeAllCouples(shoupaiOrg) {
     ...
     */
    static removeAlltripleContinues(sortedShoupais, triples) {
     ...
     * @returns
     */
    static isAlltripleContinues(shoupais) {
        let sortedShoupai = [...shoupais].sort((a, b) => a - b);
        let triples = [];
        this.removeAlltripleContinues(sortedShoupai, triples);
        let removeTripleContinuesResult = new RemoveTripleContinuesResult(sortedShoupai.length === 0, triples);
