...
export function getCardZhStr(card) {
    let number;
    if (card < 10) {
        number = card;
    }
...
export function getCardStr(card) {
    if (card < 10) {
...
export function getCardListStr(card) {
    return card.map(value => getCardStr(value));
}
export function getCardTypeStr(paiType) {
    switch (paiType) {
        case PaiType.WAN:
            return "万";
        case PaiType.TONG:
...
export function getCardTypeStrByCard(card) {
...
export function getCardListByType(cardList, paiType) {
    switch (paiType) {
        case PaiType.TONG:
